...
<?php
if ( $meta && isset( $meta ) ) {
	echo '<input class="cmb2-text-small cmb2-datepicker" type="text" name="' . esc_attr__( $field->args['id'] ) . '" id="' . esc_attr__( $field->args['id'] ) . '" value="', '' !== $meta ? esc_attr__( apply_filters( 'timeline_express_admin_render_date_format', date( 'm/d/Y' , $meta ), $meta ) ) : esc_attr__( $field->args['default'] ) . '" />';
